        ...
        <nav class="border-t border-gray-100">
            <div class="max-w-6xl mx-auto px-4">
                <ul class="flex items-center justify-center space-x-8 py-2 text-xs font-bold uppercase tracking-wider">
                    ...
                    <li><a href="{{ route('giay.index') }}" class="text-orange-500 border-b-2 border-orange-500 pb-1">GIÀY CẦU LÔNG</a></li>
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between mb-6 gap-4">
            <div>
                ...
                <p class="text-xs text-gray-500 mt-1">
                    Hiển thị {{ $danhSachGiay instanceof \Illuminate\Pagination\LengthAwarePaginator ? $danhSachGiay->total() : $danhSachGiay->count() }} sản phẩm
                </p>
            </div>

            <!-- Bộ lọc Sắp xếp -->
            <form action="{{ route('giay.index') }}" method="GET" class="flex items-center gap-2">
                @if(request('keyword'))
...
>>>>>>> Manh
            ...
            <!-- Phân trang (Nếu sử dụng paginate) -->
            @if($danhSachGiay instanceof \Illuminate\Pagination\LengthAwarePaginator)
                <div class="mt-8">
                    {{ $danhSachGiay->appends(request()->query())->links() }}
                </div>
            @endif
        @else
            <div class="bg-white rounded-lg p-12 text-center border border-gray-200">
                ...
                <a href="{{ route('giay.index') }}" class="inline-block mt-4 text-xs bg-orange-500 text-white font-bold px-4 py-2 rounded hover:bg-orange-600 transition">Xem tất cả giày</a>
            </div>
        @endif
