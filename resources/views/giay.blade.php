...
<head>
    ...
    <title>GIÀY CẦU LÔNG - BADMINTON PRO SHOP</title>
    ...
    <header class="bg-white shadow-sm sticky top-0 z-50">
        <div class="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between">
            ...
            <div class="flex-1 max-w-xl mx-8">
                <form action="{{ url('/giay-cau-long') }}" method="GET" class="flex items-center w-full border-2 border-orange-500 rounded-lg overflow-hidden bg-white shadow-sm">
    ...
    <main class="max-w-6xl mx-auto px-4 py-8">
        <div class="flex flex-col md:flex-row md:items-center justify-between mb-6 gap-4">
            <div>
                ...
                <p class="text-xs text-gray-500 mt-1">Hiển thị {{ $danhSachGiay->count() }} sản phẩm</p>
            </div>

            <!-- Bộ lọc Sắp xếp -->
            <form action="{{ url('/giay-cau-long') }}" method="GET" class="flex items-center gap-2">
                @if(request('keyword'))
        ...
        <div class="flex flex-wrap gap-2 mb-8">
            ...
            <a href="{{ url('/giay-cau-long?keyword=Yonex') }}" class="text-xs border rounded-full px-3 py-1 hover:border-orange-500 hover:text-orange-500 transition {{ request('keyword') == 'Yonex' ? 'bg-orange-500 text-white border-orange-500' : 'bg-white' }}">Yonex</a>
            <a href="{{ url('/giay-cau-long?keyword=Victor') }}" class="text-xs border rounded-full px-3 py-1 hover:border-orange-500 hover:text-orange-500 transition {{ request('keyword') == 'Victor' ? 'bg-orange-500 text-white border-orange-500' : 'bg-white' }}">Victor</a>
            <a href="{{ url('/giay-cau-long?keyword=Lining') }}" class="text-xs border rounded-full px-3 py-1 hover:border-orange-500 hover:text-orange-500 transition {{ request('keyword') == 'Lining' ? 'bg-orange-500 text-white border-orange-500' : 'bg-white' }}">Lining</a>
            <a href="{{ url('/giay-cau-long?keyword=Mizuno') }}" class="text-xs border rounded-full px-3 py-1 hover:border-orange-500 hover:text-orange-500 transition {{ request('keyword') == 'Mizuno' ? 'bg-orange-500 text-white border-orange-500' : 'bg-white' }}">Mizuno</a>
            @if(request('keyword'))
                <a href="{{ url('/giay-cau-long') }}" class="text-xs text-red-500 hover:underline self-center ml-2">Xóa bộ lọc</a>
            @endif
        </div>
        ...
        @else
            <div class="bg-white rounded-lg p-12 text-center border border-gray-200">
                ...
                <p class="text-sm text-gray-500 font-medium">Không tìm thấy sản phẩm giày cầu lông nào phù hợp.</p>
                <a href="{{ url('/giay-cau-long') }}" class="inline-block mt-4 text-xs bg-orange-500 text-white font-bold px-4 py-2 rounded hover:bg-orange-600 transition">Xem tất cả giày</a>
            </div>
        @endif
    </main>
